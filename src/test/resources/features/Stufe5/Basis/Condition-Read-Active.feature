...
Feature: Read Information from a resource of type Condition with status "active" (@Condition-Read-Active)
  ...
  Scenario: Read and Validate active Condition by its ID
    When Get FHIR resource at "http://fhirserver/Condition/${basis.condition-read-active-id}" with content type "xml"
    ...
    And resource has ID "${basis.condition-read-active-id}" with error message "The ID does not match the expected value"
    And FHIR current response body evaluates the FHIRPath "code.coding.where(system = 'http://fhir.de/CodeSystem/bfarm/icd-10-gm' and code = 'J06.9' and version = '${basis.icd-10-gm-version}').exists()" with error message "The Condition code is missing or a required value is absent"
    And element "subject" references resource with ID "${basis.patient-read-id}" with error message "The referenced case does not match the expected value."
    ...
    And FHIR current response body evaluates the FHIRPath "bodySite.coding.where(system = 'http://snomed.info/sct' and version = 'http://snomed.info/sct/11000274103/version/${basis.snomed-ct-version}' and code = '49076000').exists()" with error message 'The body site code does not match the expected value'
    ...
    And TGR current response with attribute "$.body.Condition.extension.valueReference.reference.value" matches "Condition/${basis.condition-read-resolved-id}"
