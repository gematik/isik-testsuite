...
Feature: Read information from a resource of type Observation for the expected pregnancy delivery date (@Observation-Expected-Pregnancy-Delivery-Date-Read)
  ...
  Scenario: Read and Validate expected pregnancy delivery date Observation by its ID
    When Get FHIR resource at "http://fhirserver/Observation/${data.observation-expected-pregnancy-delivery-date-read-id}" with content type "xml"
    ...
    And resource has ID "${data.observation-expected-pregnancy-delivery-date-read-id}" with error message "The ID does not match the expected value"
