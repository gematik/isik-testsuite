...
Feature: Read information from a resource of type Observation for the expected pregnancy delivery date (@Observation-Expected-Pregnancy-Delivery-Date-Read)
  ...
  Scenario: Precondition
    ...
    """
      ...
      - The ID of the corresponding FHIR resource for this test dataset must be stored in the configuration variable 'observation-expected-pregnancy-delivery-date-read-id'.

      Create an Observation resource with these fields:
      * Id: the value of the configuration variable 'observation-expected-pregnancy-delivery-date-read-id'
  ...
  Scenario: Read and Validate expected pregnancy delivery date Observation by its ID
    When Get FHIR resource at "http://fhirserver/Observation/${basis.observation-expected-pregnancy-delivery-date-read-id}" with content type "xml"
    ...
    And resource has ID "${basis.observation-expected-pregnancy-delivery-date-read-id}" with error message "The ID does not match the expected value"
    And TGR current response with attribute "$..status.value" matches "final"
    And FHIR current response body evaluates the FHIRPath "code.coding.where(system = 'http://loinc.org' and code = '11779-6' and version = '${basis.loinc-version}' and display.empty().not()).exists()" with error message 'The LOINC code for the expected pregnancy delivery date does not match the expected value'
