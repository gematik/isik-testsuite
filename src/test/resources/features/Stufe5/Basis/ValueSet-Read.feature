...
Feature: Read Information from a resource of type ValueSet (@ValueSet-Read)
  ...
  Scenario: Read and Validate ValueSet by its ID
    When Get FHIR resource at "http://fhirserver/ValueSet/${basis.valueset-read-id}" with content type "xml"
    ...
    And resource has ID "${basis.valueset-read-id}" with error message "The ID does not match the expected value"
