...
Feature: Read Information from a resource of type Condition with status "resolved" (@Condition-Read-Resolved)
  ...
  Scenario: Read and Validate resolved Condition by its ID
    When Get FHIR resource at "http://fhirserver/Condition/${basis.condition-read-resolved-id}" with content type "xml"
    ...
    And resource has ID "${basis.condition-read-resolved-id}" with error message "The ID does not match the expected value"
    And FHIR current response body evaluates the FHIRPath "code.coding.where(system = 'http://fhir.de/CodeSystem/bfarm/icd-10-gm' and code = 'M17.0' and version = '${basis.icd-10-gm-version}').exists()" with error message 'The Condition code is missing or a required value is absent'
