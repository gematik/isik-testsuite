...
Feature: Read Information from a resource of type "statutory" Coverage (@Coverage-Read-Statutory)
  ...
  Scenario: Read and Validate statutory Coverage by its ID
    When Get FHIR resource at "http://fhirserver/Coverage/${basis.coverage-read-statutory-id}" with content type "xml"
    ...
    And resource has ID "${basis.coverage-read-statutory-id}" with error message "The returned Coverage resource has not the expected ID"
    ...
    And element "beneficiary" references resource with ID "${basis.patient-read-id}" with error message "The referenced patient does not match the expected value."
    ...
    And FHIR current response body evaluates the FHIRPath "subscriber.identifier.where(system = 'http://fhir.de/sid/gkv/kvid-10' and value = '${basis.related-person-identifier-value}').exists()" with error message "The subscriber's identifier does not match the expected value"
