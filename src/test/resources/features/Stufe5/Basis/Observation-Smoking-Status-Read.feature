...
Feature: Read information from a resource of type Observation for smoking status (@Observation-Smoking-Status-Read)
  ...
  Scenario: Read and Validate smoking status Observation by its ID
    When Get FHIR resource at "http://fhirserver/Observation/${basis.observation-smoking-status-read-id}" with content type "xml"
    ...
    And resource has ID "${basis.observation-smoking-status-read-id}" with error message "The ID does not match the expected value"
    And TGR current response with attribute "$..status.value" matches "final"
    And FHIR current response body evaluates the FHIRPath "code.coding.where(system = 'http://snomed.info/sct' and code = '77176002' and version = 'http://snomed.info/sct/11000274103/version/${basis.snomed-ct-version}' and display.empty().not()).exists()" with error message 'The SNOMED CT code for smoking status does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "code.coding.where(system = 'http://loinc.org' and code = '72166-2' and version = '${basis.loinc-version}' and display.empty().not()).exists()" with error message 'The LOINC code for smoking status does not match the expected value'
    ...
    And FHIR current response body evaluates the FHIRPath "value.coding.where(system = 'http://loinc.org' and code = 'LA15920-4' and version = '${basis.loinc-version}' and display.empty().not()).exists()" with error message 'The smoking status value does not match the expected value'
