...
Feature: Read Information from a resource of type Organization (@Organization-Read) (Optional)
  ...
  Scenario: Read and Validate Organization by its ID
    When Get FHIR resource at "http://fhirserver/Organization/${basis.organization-read-id}" with content type "xml"
    ...
    And resource has ID "${basis.organization-read-id}" with error message "The ID does not match the expected value"
    ...
    And FHIR current response body evaluates the FHIRPath "identifier.where(system = 'https://fhir.krankenhaus.example/sid/OrgaID' and value = '123456' and type.coding.where(system = 'http://snomed.info/sct' and version = 'http://snomed.info/sct/11000274103/version/${basis.snomed-ct-version}' and code = '225746001')).exists()" with error message 'The Organization Unit ID does not match the expected value'
    ...
    And FHIR current response body evaluates the FHIRPath "partOf.where(reference = 'Organization/${basis.organization-read-parent-id}').exists()" with error message 'The Organization partOf reference does not match the expected value'
