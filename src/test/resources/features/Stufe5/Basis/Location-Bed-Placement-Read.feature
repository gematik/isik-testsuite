...
Feature: Read Information from a resource of type Location that identifies the placement of beds (@Location-Bed-Placement-Read) (Optional)
  ...
  Scenario: Read and Validate Bed Placement Location by its ID
    When Get FHIR resource at "http://fhirserver/Location/${basis.location-bed-placement-read-id}" with content type "xml"
    ...
    And resource has ID "${basis.location-bed-placement-read-id}" with error message "The ID does not match the expected value"
    ...
    And FHIR current response body evaluates the FHIRPath "managingOrganization.where(reference = 'Organization/${basis.organization-read-id}').exists()" with error message 'The Location managing organization reference does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "partOf.where(reference = 'Location/${basis.location-room-read-id}').exists()" with error message 'The Location partOf reference does not match the expected value'
