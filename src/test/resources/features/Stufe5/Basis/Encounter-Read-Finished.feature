...
Feature: Read Information from a resource of type Encounter with status "finished" (@Encounter-Read-Finished)
  ...
  Scenario: Read and Validate finished Encounter by its ID
    When Get FHIR resource at "http://fhirserver/Encounter/${basis.encounter-read-finished-id}" with content type "xml"
    ...
    And resource has ID "${basis.encounter-read-finished-id}" with error message "The returned Encounter resource has not the expected ID"
    And TGR current response with attribute "$..status.value" matches "finished"
    And FHIR current response body evaluates the FHIRPath "identifier.where(system = '${basis.encounter-read-finished-identifier-system}' and value='${basis.encounter-read-finished-identifier-value}').exists()" with error message 'The Encounter does not contain the correct admission number'
    ...
    And element "subject" references resource with ID "${basis.account-read-patient-id}" with error message "The referenced Patient does not match the expected value"
    ...
    And element "account" references resource with ID "Account/${basis.account-read-id}" with error message "The linked billing case does not match the expected value"
    And FHIR current response body evaluates the FHIRPath "account.identifier.value = '${basis.account-read-identifier-value}'" with error message 'The identifier of the linked billing case does not match the expected value'
