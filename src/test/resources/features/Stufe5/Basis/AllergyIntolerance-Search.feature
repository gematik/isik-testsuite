...
Feature: Testing search parameters against a resource of type AllergyIntolerance (@AllergyIntolerance-Search)
  ...
  Scenario: Search for the AllergyIntolerance resource by ID
    When Get FHIR resource at "http://fhirserver/AllergyIntolerance/?_id=${basis.allergyintolerance-read-id}" with content type "xml"
    And FHIR current response body is a valid CORE resource and conforms to profile "https://hl7.org/fhir/StructureDefinition/Bundle"
    And response bundle contains resource with ID "${basis.allergyintolerance-read-id}" with error message "The requested AllergyIntolerance ${basis.allergyintolerance-read-id} is not contained in the response bundle."
  ...
  Scenario: Search for the AllergyIntolerance resources by Patient, group by _count
    When Get FHIR resource at "http://fhirserver/AllergyIntolerance/?_count=${basis.search-count}&patient=Patient/${basis.allergyintolerance-read-patient-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0 and entry.resource.count() <= ${basis.search-count}' with error message 'No search results were found'
    And element "patient" in all bundle resources references resource with ID "${basis.allergyintolerance-read-patient-id}" with error message 'There are search results, but they do not fully match the search criteria'

  Scenario: Search for the AllergyIntolerance resources that have the Category 'environment', group by _count
    When Get FHIR resource at "http://fhirserver/AllergyIntolerance/?_count=${basis.search-count}&category=environment" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0 and entry.resource.count() <= ${basis.search-count}' with error message 'No search results were found'
  ...
  Scenario: Search for the AllergyIntolerance by registered Date (registered) after 2026-01-01, group by _count
    When Get FHIR resource at "http://fhirserver/AllergyIntolerance/?date=ge2026-01-01&_count=${basis.search-count}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0 and entry.resource.count() <= ${basis.search-count}' with error message 'No search results were found'
