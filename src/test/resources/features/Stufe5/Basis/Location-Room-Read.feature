...
Feature: Read Information from a resource of type Location that identifies a room (@Location-Room-Read) (Optional)
  ...
  Scenario: Read and Validate Room Location by its ID
    When Get FHIR resource at "http://fhirserver/Location/${basis.location-room-read-id}" with content type "xml"
    ...
    And resource has ID "${basis.location-room-read-id}" with error message "The ID does not match the expected value"
    ...
    And FHIR current response body evaluates the FHIRPath "partOf.where(reference = 'Location/${basis.location-read-id}').exists()" with error message 'The Location partOf reference does not match the expected value'
