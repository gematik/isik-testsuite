...
Feature: Test the Access to a Patient Resource with the Patient Scope (@Patient-Read-Scope)
  ...
  Scenario: Search for Patients with the patient/Patient.rs scope returns only one Patient
    ...
    Then FHIR current response body evaluates the FHIRPath "Bundle.entry.resource.ofType(Patient).where(id.replaceMatches('/_history/.*', '').replaceMatches('^.*/', '') = '${connect.read-patient-in-context-id}').exists()" with error message "Expected the specific Patient resource to exist in the bundle"

  Scenario: Access to Condition resources of patients not in context is forbidden
    # patient/Patient.rs grants no Condition access, regardless of patient context.
    # Accept refusal or an empty search result; neither may expose Condition resources.
