...
Feature: Testing search parameters against a resource of type Observation (@Observation-Search)
  ...
  Scenario: Search for the Observation by ID
    Then Get FHIR resource at "http://fhirserver/Observation/?_id=${icuminimal.observation-read-arterial-blood-pressure-id}" with content type "xml"
    And response bundle contains resource with ID "${icuminimal.observation-read-arterial-blood-pressure-id}" with error message "The requested Observation ${icuminimal.observation-read-arterial-blood-pressure-id} is not contained in the response bundle"
  ...
  Scenario: Search for the Observation by Tag and ID
    When Get FHIR resource at "http://fhirserver/Observation/?_tag=${icuminimal.tag-system}%7C${icuminimal.tag-value}&_id=${icuminimal.observation-read-arterial-blood-pressure-id}" with content type "xml"
  ...
  Scenario: Search for the Observation that belong to a Patient, by Status
    Then Get FHIR resource at "http://fhirserver/Observation/?status=final&patient=Patient/${icuminimal.observation-patient-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
  ...
  Scenario: Search for the Observation that belong to a Patient, by Category
    Then Get FHIR resource at "http://fhirserver/Observation/?category=vital-signs&patient=Patient/${icuminimal.observation-patient-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
  ...
  Scenario: Search for the Observation that belong to a Patient, by Date
    Then Get FHIR resource at "http://fhirserver/Observation/?date=2026-03-05&patient=Patient/${icuminimal.observation-patient-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
  ...
  Scenario: Search for the Observation that belong to a Patient, by Date with 'le' Modifier
    Then Get FHIR resource at "http://fhirserver/Observation/?patient=Patient/${icuminimal.observation-patient-id}&date=le2027-01-01" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
  ...
  Scenario: Search for the Observation that belong to a Patient, by Code
    Then Get FHIR resource at "http://fhirserver/Observation/?code=85354-9&patient=Patient/${icuminimal.observation-patient-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
  ...
  Scenario: Search for the Observation by Patient (Search Parameter 'patient')
    Then Get FHIR resource at "http://fhirserver/Observation/?patient=Patient/${icuminimal.observation-patient-id}" with content type "json"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
    And element "subject" in all bundle resources references resource with ID "Patient/${icuminimal.observation-patient-id}"

  @Optional
  Scenario: Optional Search for the Observation by Patient (Search Parameter 'subject')
    Then Get FHIR resource at "http://fhirserver/Observation/?subject=Patient/${icuminimal.observation-patient-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
    And element "subject" in all bundle resources references resource with ID "Patient/${icuminimal.observation-patient-id}"

  Scenario: Search for the Observation that belong to a Patient, by Encounter
    Then Get FHIR resource at "http://fhirserver/Observation/?encounter=Encounter/${icuminimal.observation-encounter-id}&patient=Patient/${icuminimal.observation-patient-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
    And FHIR current response body evaluates the FHIRPath "entry.resource.all(encounter.reference.replaceMatches('/_history/.+','').matches('\\b${icuminimal.observation-encounter-id}$'))" with error message 'There are search results, but they do not fully match the search criteria.'

  Scenario: Search for the Observation that belong to a Patient, by Combo Code
    Then Get FHIR resource at "http://fhirserver/Observation/?combo-code=8462-4&patient=Patient/${icuminimal.observation-patient-id}" with content type "json"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
  ...
  Scenario: Search for the Observation that belong to a Patient, by Combo Code Value Quantity
    Then Get FHIR resource at "http://fhirserver/Observation/?patient=Patient/${icuminimal.observation-patient-id}&combo-code-value-quantity=8480-6$107" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
  ...
  Scenario: Search for the Observation that belong to a Patient, by Component Code
    Then Get FHIR resource at "http://fhirserver/Observation/?component-code=8480-6&patient=Patient/${icuminimal.observation-patient-id}" with content type "json"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
