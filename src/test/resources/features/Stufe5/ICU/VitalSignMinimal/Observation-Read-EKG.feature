...
Feature: Read Information from a resource of type Observation for ECG data (@Observation-Read-EKG)
  ...
  Scenario: Read and Validate the Observation by its ID
    Then Get FHIR resource at "http://fhirserver/Observation/${icuminimal.observation-read-ekg-id}" with content type "xml"
    And resource has ID "${icuminimal.observation-read-ekg-id}"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = 'LP6244-0' and system = 'http://loinc.org' and version = '${icuminimal.loinc-version}' and display.empty().not()).exists()" with error message 'The code does not match the expected value'
    ...
    And FHIR current response body evaluates the FHIRPath "component.where(code.coding.where(code = 'LP7386-8' and system = 'http://loinc.org' and version = '${icuminimal.loinc-version}' and display.empty().not()).exists()).exists()" with error message 'The first ECG lead is missing'
    And FHIR current response body evaluates the FHIRPath "component.where(code.coding.where(code = 'LP7387-6' and system = 'http://loinc.org' and version = '${icuminimal.loinc-version}' and display.empty().not()).exists()).exists()" with error message 'The second ECG lead is missing'
    And FHIR current response body evaluates the FHIRPath "component.where(code.coding.where(code = 'LP7388-4' and system = 'http://loinc.org' and version = '${icuminimal.loinc-version}' and display.empty().not()).exists()).exists()" with error message 'The third ECG lead is missing'
