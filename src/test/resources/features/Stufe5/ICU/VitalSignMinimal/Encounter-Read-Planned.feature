...
Feature: Read Information from a resource of type Encounter with status "planned" (@Encounter-Read-Planned)
  ...
  Scenario: Read and Validate planned Encounter by its ID
    When Get FHIR resource at "http://fhirserver/Encounter/${icuminimal.encounter-read-planned-id}" with content type "xml"
    ...
    And resource has ID "${icuminimal.encounter-read-planned-id}" with error message "The returned Encounter resource has not the expected ID"
    And TGR current response with attribute "$..status.value" matches "planned"
    And FHIR current response body evaluates the FHIRPath "identifier.where(system = '${icuminimal.encounter-read-planned-identifier-system}' and value='${icuminimal.encounter-read-planned-identifier-value}').exists()" with error message 'The Encounter does not contain the correct admission number'
    ...
    And element "subject" references resource with ID "${icuminimal.patient-read-id}" with error message "The referenced Patient does not match the expected value"
    ...
    And FHIR current response body evaluates the FHIRPath "serviceProvider.identifier.value = '${icuminimal.encounter-read-planned-serviceprovider-identifier-value}'" with error message 'The service provider identifier value does not match the expected value'
