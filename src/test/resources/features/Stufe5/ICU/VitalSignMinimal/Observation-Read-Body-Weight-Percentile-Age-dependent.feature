...
@Observation-Read-Body-Weight-Percentile-Age-dependent
Feature: Read Information from a resource of type Observation for Body Weight Percentile Age-dependent data (@Observation-Read-Body-Weight-Percentile-Age-dependent)
  ...
  Scenario: Precondition
    ...
    """
      ...
      - The ID of the corresponding FHIR resource for this test data set must be stored in the configuration variable 'observation-read-body-weight-percentile-age-dependent-id'.

      Create an Observation resource in your system with the following values:
      * Id: the value of the configuration variable 'observation-read-body-weight-percentile-age-dependent-id'
      * Profile: sd-mii-icu-koerpergewicht-percentil-altersabhaengig
      * Status: final
      * Identifier:
        - System: https://fhir.krankenhaus.example/sid/observation
          Value: ICUmin-OBS-025
      * Category:
        - Coding:
          * Code: vital-signs
      ...
      * Effective date/time: 2026-03-12T08:15:00+01:00
      * Value:
        - Quantity:
          * Value: 75
          * Unit: percent
  ...
  Scenario: Read and Validate the Observation by its ID
    Then Get FHIR resource at "http://fhirserver/Observation/${icuminimal.observation-read-body-weight-percentile-age-dependent-id}" with content type "xml"
    And resource has ID "${icuminimal.observation-read-body-weight-percentile-age-dependent-id}"
    And FHIR current response body is a valid isik5 resource and conforms to profile "https://gematik.de/fhir/isik/StructureDefinition/sd-mii-icu-koerpergewicht-percentil-altersabhaengig"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '8336-0' and system = 'http://loinc.org' and version = '${icuminimal.loinc-version}' and display.empty().not()).exists()" with error message 'The code does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '1153592008' and system = 'http://snomed.info/sct' and version = 'http://snomed.info/sct/11000274103/version/${icuminimal.snomed-ct-version}' and display.empty().not()).exists()" with error message 'The code does not match the expected value'
    ...
    And FHIR current response body evaluates the FHIRPath "effective.toString().contains('2026-03-12T08:15:00+01:00')" with error message 'The effective date does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "value.value = 75" with error message 'The value does not match the expected value'
