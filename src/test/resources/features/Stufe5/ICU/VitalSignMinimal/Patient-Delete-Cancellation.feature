...
Feature: Delete Patient (@Patient-Delete-Cancellation) (Optional)
  ...
  Scenario: Cancellation of a patient via Delete
    ...
    When TGR send empty DELETE request to "http://fhirserver/Patient/${icuminimal.patient-delete-id}"
    ...
    And TGR send empty GET request to "http://fhirserver/Patient/${icuminimal.patient-delete-id}"
    And TGR find the last request
    # Ignore content-type header, as it may be missing for 4** responses
