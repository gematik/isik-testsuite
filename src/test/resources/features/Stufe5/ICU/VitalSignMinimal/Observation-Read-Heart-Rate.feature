...
Feature: Read Information from a resource of type Observation for heart rate (@Observation-Read-Heart-Rate)
  ...
  Scenario: Read and Validate the Observation by its ID
    Then Get FHIR resource at "http://fhirserver/Observation/${icuminimal.observation-read-heart-rate-id}" with content type "xml"
    And resource has ID "${icuminimal.observation-read-heart-rate-id}"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '8867-4' and system = 'http://loinc.org' and version = '${icuminimal.loinc-version}' and display.empty().not()).exists()" with error message 'The code does not match the expected value'
  ...
  Scenario: Read optional Method from Observation
    Then Get FHIR resource at "http://fhirserver/Observation/${icuminimal.observation-read-heart-rate-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath "method.coding.where(code = '268400002' and system = 'http://snomed.info/sct' and version = 'http://snomed.info/sct/11000274103/version/${icuminimal.snomed-ct-version}' and display.empty().not()).exists()" with error message 'The method does not match the expected value'
