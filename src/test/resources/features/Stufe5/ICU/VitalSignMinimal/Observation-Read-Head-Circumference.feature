...
Feature: Read Information from a resource of type Observation for head circumference (@Observation-Read-Kopfumfang)
  ...
  Scenario: Read and Validate the Observation by its ID
    Then Get FHIR resource at "http://fhirserver/Observation/${icuminimal.observation-read-head-circumference-id}" with content type "xml"
    And resource has ID "${icuminimal.observation-read-head-circumference-id}"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '9843-4' and system = 'http://loinc.org' and version = '${icuminimal.loinc-version}' and display.empty().not()).exists()" with error message 'The code does not match the expected value'
