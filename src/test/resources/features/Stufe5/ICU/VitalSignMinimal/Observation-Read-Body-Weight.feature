...
Feature: Read Information from a resource of type Observation for body weight (@Observation-Read-Body-Weight)
  ...
  Scenario: Read and Validate the Observation by its ID
    Then Get FHIR resource at "http://fhirserver/Observation/${icuminimal.observation-read-body-weight-id}" with content type "xml"
    And resource has ID "${icuminimal.observation-read-body-weight-id}"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '29463-7' and system = 'http://loinc.org' and version = '${icuminimal.loinc-version}' and display.empty().not()).exists()" with error message 'The code does not match the expected value'
