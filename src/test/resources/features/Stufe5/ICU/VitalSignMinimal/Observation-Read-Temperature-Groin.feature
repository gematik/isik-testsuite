...
@Observation-Read-Temperature-Groin
Feature: Read Information from a resource of type Observation for groin temperature data (@Observation-Read-Temperature-Leiste)
  ...
  Scenario: Precondition
    ...
    """
      ...
      - The ID of the corresponding FHIR resource for this test data set must be stored in the configuration variable 'observation-read-temperature-leiste-id'.

      Create an Observation resource in your system with the following values:
      * Id: the value of the configuration variable 'observation-read-temperature-leiste-id'
      * Profile: sd-mii-icu-koerpertemperatur-leiste
      * Status: final
      * Identifier:
        - System: https://fhir.krankenhaus.example/sid/observation
          Value: ICUmin-OBS-007
      * Category:
        - Coding:
          * Code: vital-signs
      ...
      * Effective date/time: 2026-03-12T08:42:00+01:00
      * Body Site:
        - Coding:
          * Code: 26893007
  ...
  Scenario: Read and Validate the Observation by its ID
    Then Get FHIR resource at "http://fhirserver/Observation/${icuminimal.observation-read-temperature-leiste-id}" with content type "xml"
    And resource has ID "${icuminimal.observation-read-temperature-leiste-id}"
    And FHIR current response body is a valid isik5 resource and conforms to profile "https://gematik.de/fhir/isik/StructureDefinition/sd-mii-icu-koerpertemperatur-leiste"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '415929009' and system = 'http://snomed.info/sct' and version = 'http://snomed.info/sct/11000274103/version/${icuminimal.snomed-ct-version}' and display.empty().not()).exists()" with error message 'The SNOMED code does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '104063-3' and system = 'http://loinc.org' and version = '${icuminimal.loinc-version}' and display.empty().not()).exists()" with error message 'The LOINC code does not match the expected value'
    ...
    And FHIR current response body evaluates the FHIRPath "effective.toString().contains('2026-03-12T08:42:00+01:00')" with error message 'The effective date does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "value.value = 36.7" with error message 'The value does not match the expected value'
    ...
    And FHIR current response body evaluates the FHIRPath "bodySite.coding.where(code = '26893007' and system = 'http://snomed.info/sct' and version = 'http://snomed.info/sct/11000274103/version/${icuminimal.snomed-ct-version}' and display.empty().not()).exists()" with error message 'The body site does not match the expected value'
