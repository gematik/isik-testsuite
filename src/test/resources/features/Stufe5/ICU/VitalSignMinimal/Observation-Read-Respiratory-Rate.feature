...
Feature: Read Information from a resource of type Observation for respiratory rate (@Observation-Read-Respiratory-Rate)
  ...
  Scenario: Read and Validate the Observation by its ID
    Then Get FHIR resource at "http://fhirserver/Observation/${icuminimal.observation-read-respiratory-rate-id}" with content type "xml"
    And resource has ID "${icuminimal.observation-read-respiratory-rate-id}"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '9279-1' and system = 'http://loinc.org' and version = '${icuminimal.loinc-version}' and display.empty().not()).exists()" with error message 'The code does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "method.coding.where(code = '86290005' and system = 'http://snomed.info/sct' and version = 'http://snomed.info/sct/11000274103/version/${icuminimal.snomed-ct-version}' and display.empty().not()).exists()" with error message 'The method does not match the expected value'
    ...
    And referenced "Patient" resource with id "${icuminimal.observation-patient-id}" conforms to a valid v5 "ISiKPatient" profile
    And referenced "Encounter" resource with id "${icuminimal.observation-encounter-id}" conforms to a valid v5 "ISiKKontaktGesundheitseinrichtung" profile
    And referenced "Practitioner" resource with id "${icuminimal.observation-practitioner-id}" conforms to a valid v5 "ISiKPersonImGesundheitsberuf" profile
