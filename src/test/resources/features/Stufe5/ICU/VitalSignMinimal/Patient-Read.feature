...
Feature: Read Information from a resource of type Patient (@Patient-Read)
  ...
  Scenario: Read and Validate Patient by their ID
    When Get FHIR resource at "http://fhirserver/Patient/${icuminimal.patient-read-id}" with content type "xml"
    ...
    And resource has ID "${icuminimal.patient-read-id}" with error message "The ID does not match the expected value"
