...
Feature: Read Information from a resource of type Encounter with status "in progress" (@Encounter-Read-In-Progress)
  ...
  Scenario: Read and Validate in-progress Encounter by its ID
    When Get FHIR resource at "http://fhirserver/Encounter/${icuminimal.encounter-read-in-progress-id}" with content type "xml"
    ...
    And resource has ID "${icuminimal.encounter-read-in-progress-id}" with error message "The returned Encounter resource has not the expected ID"
    And TGR current response with attribute "$..status.value" matches "in-progress"
    And FHIR current response body evaluates the FHIRPath "identifier.where(system = '${icuminimal.encounter-read-in-progress-identifier-system}' and value='${icuminimal.encounter-read-in-progress-identifier-value}').exists()" with error message 'The Encounter does not contain the correct admission number'
    ...
    And element "diagnosis.condition" references resource with ID "Condition/${icuminimal.condition-read-active-id}" with error message "The referenced Condition does not match the expected value."
