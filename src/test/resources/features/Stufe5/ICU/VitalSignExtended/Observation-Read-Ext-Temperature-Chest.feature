...
@Observation-Read-Ext-Temperature-Chest
Feature: Read Information from a resource of type Observation for chest temperature data (@Observation-Read-Ext-Temperature-Chest)
  ...
  Scenario: Precondition
    ...
    """
      ...
      - The ID of the corresponding FHIR resource for this test data set must be stored in the configuration variable 'observation-read-ext-temperature-chest-id'.

      Create an Observation resource in your system with the following values:
      * Id: the value of the configuration variable 'observation-read-ext-temperature-chest-id'
      * Profile: sd-mii-icu-koerpertemperatur-brust
      * Status: final
      * Category:
        - Coding:
          * Code: vital-signs
  ...
  Scenario: Read and Validate the Observation by its ID
    Then Get FHIR resource at "http://fhirserver/Observation/${icuextended.observation-read-ext-temperature-chest-id}" with content type "xml"
    And resource has ID "${icuextended.observation-read-ext-temperature-chest-id}"
    And FHIR current response body is a valid isik5 resource and conforms to profile "https://gematik.de/fhir/isik/StructureDefinition/sd-mii-icu-koerpertemperatur-brust"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '248835004' and system = 'http://snomed.info/sct' and version = 'http://snomed.info/sct/11000274103/version/${icuextended.snomed-ct-version}' and display.empty().not()).exists()" with error message 'The SNOMED code does not match the expected value'
    ...
    And FHIR current response body evaluates the FHIRPath "bodySite.coding.where(code = '76752008' and system = 'http://snomed.info/sct' and version = 'http://snomed.info/sct/11000274103/version/${icuextended.snomed-ct-version}' and display.empty().not()).exists()" with error message 'The body site does not match the expected value'
