...
@Observation-Read-Ext-LeftVentricularCardiacIndexThermodilution
Feature: Read Information from a resource of type Observation for left ventricular cardiac index by thermodilution data (@Observation-Read-Ext-LeftVentricularCardiacIndexThermodilution)
  ...
  Scenario: Precondition
    ...
    """
      ...
      - The ID of the corresponding FHIR resource for this test data set must be stored in the configuration variable 'observation-read-ext-lv-cardiac-index-thermodilution-id'.

      Create an Observation resource in your system with the following values:
      * Id: the value of the configuration variable 'observation-read-ext-lv-cardiac-index-thermodilution-id'
      * Profile: sd-mii-icu-linksventri-herzindex-durch-indikatorverduennung
      * Status: final
      * Category:
        - Coding:
          * Code: vital-signs
  ...
  Scenario: Read and Validate the Observation by its ID
    Then Get FHIR resource at "http://fhirserver/Observation/${icuextended.observation-read-ext-lv-cardiac-index-thermodilution-id}" with content type "xml"
    And resource has ID "${icuextended.observation-read-ext-lv-cardiac-index-thermodilution-id}"
    And FHIR current response body is a valid isik5 resource and conforms to profile "https://gematik.de/fhir/isik/StructureDefinition/sd-mii-icu-linksventri-herzindex-durch-indikatorverduennung"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '8751-0' and system = 'http://loinc.org' and version = '${icuextended.loinc-version}' and display.empty().not()).exists()" with error message 'The LOINC code does not match the expected value'
