...
@Observation-Read-Ext-Temperature-Nasopharynx
Feature: Read Information from a resource of type Observation for nasopharyngeal temperature data (@Observation-Read-Ext-Temperature-Nasopharynx)
  ...
  Scenario: Precondition
    ...
    """
      ...
      - The ID of the corresponding FHIR resource for this test data set must be stored in the configuration variable 'observation-read-ext-temperature-nasopharynx-id'.

      Create an Observation resource in your system with the following values:
      * Id: the value of the configuration variable 'observation-read-ext-temperature-nasopharynx-id'
      * Profile: sd-mii-icu-koerpertemperatur-nasen-rachen-raum
      * Status: final
      * Category:
        - Coding:
          * Code: vital-signs
  ...
  Scenario: Read and Validate the Observation by its ID
    Then Get FHIR resource at "http://fhirserver/Observation/${icuextended.observation-read-ext-temperature-nasopharynx-id}" with content type "xml"
    And resource has ID "${icuextended.observation-read-ext-temperature-nasopharynx-id}"
    And FHIR current response body is a valid isik5 resource and conforms to profile "https://gematik.de/fhir/isik/StructureDefinition/sd-mii-icu-koerpertemperatur-nasen-rachen-raum"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '698831002' and system = 'http://snomed.info/sct' and version = 'http://snomed.info/sct/11000274103/version/${icuextended.snomed-ct-version}' and display.empty().not()).exists()" with error message 'The SNOMED code does not match the expected value'
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '150380' and system = 'urn:iso:std:iso:11073:10101' and display.empty().not()).exists()" with error message 'The ISO 11073 code does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '60838-0' and system = 'http://loinc.org' and version = '${icuextended.loinc-version}' and display.empty().not()).exists()" with error message 'The LOINC code (60838-0) does not match the expected value'
    ...
    And FHIR current response body evaluates the FHIRPath "bodySite.coding.where(code = '71836000' and system = 'http://snomed.info/sct' and version = 'http://snomed.info/sct/11000274103/version/${icuextended.snomed-ct-version}' and display.empty().not()).exists()" with error message 'The body site does not match the expected value'
