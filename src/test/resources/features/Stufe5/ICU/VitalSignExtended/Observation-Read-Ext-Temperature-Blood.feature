...
@Observation-Read-Ext-Temperature-Blood
Feature: Read Information from a resource of type Observation for blood temperature data (@Observation-Read-Ext-Temperature-Blood)
  ...
  Scenario: Precondition
    ...
    """
      ...
      - The ID of the corresponding FHIR resource for this test data set must be stored in the configuration variable 'observation-read-ext-temperature-blood-id'.

      Create an Observation resource in your system with the following values:
      * Id: the value of the configuration variable 'observation-read-ext-temperature-blood-id'
      * Profile: sd-mii-icu-koerpertemperatur-blut
      * Status: final
      * Category:
        - Coding:
          * Code: vital-signs
  ...
  Scenario: Read and Validate the Observation by its ID
    Then Get FHIR resource at "http://fhirserver/Observation/${icuextended.observation-read-ext-temperature-blood-id}" with content type "xml"
    And resource has ID "${icuextended.observation-read-ext-temperature-blood-id}"
    And FHIR current response body is a valid isik5 resource and conforms to profile "https://gematik.de/fhir/isik/StructureDefinition/sd-mii-icu-koerpertemperatur-blut"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '1222808002' and system = 'http://snomed.info/sct' and version = 'http://snomed.info/sct/11000274103/version/${icuextended.snomed-ct-version}' and display.empty().not()).exists()" with error message 'The SNOMED code does not match the expected value'
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '60834-9' and system = 'http://loinc.org' and version = '${icuextended.loinc-version}' and display.empty().not()).exists()" with error message 'The LOINC code (60834-9) does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '188436' and system = 'urn:iso:std:iso:11073:10101' and display.empty().not()).exists()" with error message 'The ISO 11073 code does not match the expected value'
