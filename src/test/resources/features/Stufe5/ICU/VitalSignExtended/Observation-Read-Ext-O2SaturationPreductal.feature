...
@Observation-Read-Ext-O2SaturationPreductal
Feature: Read Information from a resource of type Observation for preductal O2 saturation data (@Observation-Read-Ext-O2SaturationPreductal)
  ...
  Scenario: Precondition
    ...
    """
      ...
      - The ID of the corresponding FHIR resource for this test data set must be stored in the configuration variable 'observation-read-ext-o2-saturation-preductal-id'.

      Create an Observation resource in your system with the following values:
      * Id: the value of the configuration variable 'observation-read-ext-o2-saturation-preductal-id'
      * Profile: sd-mii-icu-o2saettigung-im-blut-preduktal-durch-pulsoxymetrie
      * Status: final
      * Category:
        - Coding:
          * Code: vital-signs
  ...
  Scenario: Read and Validate the Observation by its ID
    Then Get FHIR resource at "http://fhirserver/Observation/${icuextended.observation-read-ext-o2-saturation-preductal-id}" with content type "xml"
    And resource has ID "${icuextended.observation-read-ext-o2-saturation-preductal-id}"
    And FHIR current response body is a valid isik5 resource and conforms to profile "https://gematik.de/fhir/isik/StructureDefinition/sd-mii-icu-o2saettigung-im-blut-preduktal-durch-pulsoxymetrie"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '59407-7' and system = 'http://loinc.org' and version = '${icuextended.loinc-version}' and display.empty().not()).exists()" with error message 'The LOINC code does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '160296' and system = 'urn:iso:std:iso:11073:10101' and display.empty().not()).exists()" with error message 'The ISO 11073 code does not match the expected value'
    ...
    And FHIR current response body evaluates the FHIRPath "value.value = 96" with error message 'The value does not match the expected value'
