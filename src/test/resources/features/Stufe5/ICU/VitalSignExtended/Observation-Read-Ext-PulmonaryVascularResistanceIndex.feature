...
@Observation-Read-Ext-PulmonaryVascularResistanceIndex
Feature: Read Information from a resource of type Observation for pulmonary vascular resistance index data (@Observation-Read-Ext-PulmonaryVascularResistanceIndex)
  ...
  Scenario: Precondition
    ...
    """
      ...
      - The ID of the corresponding FHIR resource for this test data set must be stored in the configuration variable 'observation-read-ext-pulmonary-vascular-resistance-index-id'.

      Create an Observation resource in your system with the following values:
      * Id: the value of the configuration variable 'observation-read-ext-pulmonary-vascular-resistance-index-id'
      * Profile: sd-mii-icu-pulmonalvaskulaerer-widerstandsindex
      * Status: final
      * Category:
        - Coding:
          * Code: vital-signs
  ...
  Scenario: Read and Validate the Observation by its ID
    Then Get FHIR resource at "http://fhirserver/Observation/${icuextended.observation-read-ext-pulmonary-vascular-resistance-index-id}" with content type "xml"
    And resource has ID "${icuextended.observation-read-ext-pulmonary-vascular-resistance-index-id}"
    And FHIR current response body is a valid isik5 resource and conforms to profile "https://gematik.de/fhir/isik/StructureDefinition/sd-mii-icu-pulmonalvaskulaerer-widerstandsindex"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '276902009' and system = 'http://snomed.info/sct' and version = 'http://snomed.info/sct/11000274103/version/${icuextended.snomed-ct-version}' and display.empty().not()).exists()" with error message 'The SNOMED code does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '8834-4' and system = 'http://loinc.org' and version = '${icuextended.loinc-version}' and display.empty().not()).exists()" with error message 'The LOINC code does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '152852' and system = 'urn:iso:std:iso:11073:10101' and display.empty().not()).exists()" with error message 'The ISO 11073 code does not match the expected value'
    ...
    And FHIR current response body evaluates the FHIRPath "value.value = 80" with error message 'The value does not match the expected value'
