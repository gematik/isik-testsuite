...
@Observation-Read-Ext-LeftVentricularStrokeVolumeIndex
Feature: Read Information from a resource of type Observation for left ventricular stroke volume index data (@Observation-Read-Ext-LeftVentricularStrokeVolumeIndex)
  ...
  Scenario: Precondition
    ...
    """
      ...
      - The ID of the corresponding FHIR resource for this test data set must be stored in the configuration variable 'observation-read-ext-left-ventricular-stroke-volume-index-id'.

      Create an Observation resource in your system with the following values:
      * Id: the value of the configuration variable 'observation-read-ext-left-ventricular-stroke-volume-index-id'
      * Profile: sd-mii-icu-linksventrikulaeres-schlagvolumenindex
      * Status: final
      * Category:
        - Coding:
          * Code: vital-signs
  ...
  Scenario: Read and Validate the Observation by its ID
    Then Get FHIR resource at "http://fhirserver/Observation/${icuextended.observation-read-ext-left-ventricular-stroke-volume-index-id}" with content type "xml"
    And resource has ID "${icuextended.observation-read-ext-left-ventricular-stroke-volume-index-id}"
    And FHIR current response body is a valid isik5 resource and conforms to profile "https://gematik.de/fhir/isik/StructureDefinition/sd-mii-icu-linksventrikulaeres-schlagvolumenindex"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '76297-1' and system = 'http://loinc.org' and version = '${icuextended.loinc-version}' and display.empty().not()).exists()" with error message 'The LOINC code does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '150636' and system = 'urn:iso:std:iso:11073:10101' and display.empty().not()).exists()" with error message 'The ISO 11073 code does not match the expected value'
    ...
    And FHIR current response body evaluates the FHIRPath "value.value = 40" with error message 'The value does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "value.unit = 'milliliter per square meter'" with error message 'The unit does not match the expected value'
    ...
    And FHIR current response body evaluates the FHIRPath "value.code = 'mL/m2'" with error message 'The UCUM code does not match the expected value'
