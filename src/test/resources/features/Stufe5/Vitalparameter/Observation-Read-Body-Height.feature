...
Feature: Read Information from a resource of type Observation for body height (@Observation-Read-Body-Height)
  ...
  Scenario: Read and Validate the Observation by its ID
    Then Get FHIR resource at "http://fhirserver/Observation/${vitalparameter.observation-read-body-height-id}" with content type "xml"
    And resource has ID "${vitalparameter.observation-read-body-height-id}"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '8302-2' and system = 'http://loinc.org' and version = '${vitalparameter.loinc-version}' and display.empty().not()).exists()" with error message 'The code does not match the expected value'
