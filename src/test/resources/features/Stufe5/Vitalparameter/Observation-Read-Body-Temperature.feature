...
@Observation-Read-Body-Temperature
Feature: Read Information from a resource of type Observation for body temperature (@Observation-Read-Body-Temperature)
  ...
  Scenario: Precondition
    ...
    """
      ...
      - The ID of the corresponding FHIR resource for this test data set must be stored in the configuration variable 'observation-read-body-temperature-id'.

      Create an Observation resource in your system with the following values:
      * Id: the value of the configuration variable 'observation-read-body-temperature-id'
  ...
  Scenario: Read and Validate the Observation by its ID
    Then Get FHIR resource at "http://fhirserver/Observation/${vitalparameter.observation-read-body-temperature-id}" with content type "xml"
    And resource has ID "${vitalparameter.observation-read-body-temperature-id}"
    And FHIR current response body is a valid isik5 resource and conforms to profile "https://gematik.de/fhir/isik/StructureDefinition/ISiKKoerpertemperatur"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '8310-5' and system = 'http://loinc.org' and version = '${vitalparameter.loinc-version}' and display.empty().not()).exists()" with error message 'The code does not match the expected value'
    ...
    And FHIR current response body evaluates the FHIRPath "value.where(value ~ 36.8 and code = 'Cel' and unit.exists() and system = 'http://unitsofmeasure.org').exists()" with error message 'The value does not match the expected value'
