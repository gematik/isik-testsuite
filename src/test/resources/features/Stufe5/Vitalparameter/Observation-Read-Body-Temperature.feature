...
Feature: Read Information from a resource of type Observation for body temperature (@Observation-Read-Body-Temperature)
  ...
  Scenario: Read and Validate the Observation by its ID
    Then Get FHIR resource at "http://fhirserver/Observation/${data.observation-read-body-temperature-id}" with content type "xml"
    And resource has ID "${data.observation-read-body-temperature-id}"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '8310-5' and system = 'http://loinc.org' and version = '${data.loinc-version}' and display.empty().not()).exists()" with error message 'The code does not match the expected value'
