...
Feature: Read Information from a resource of type Observation for arterial blood pressure (@Observation-Read-Arterial-Blood-Pressure)
  ...
  Scenario: Read and Validate the Observation by its ID
    Then Get FHIR resource at "http://fhirserver/Observation/${vitalparameter.observation-read-arterial-blood-pressure-id}" with content type "xml"
    And resource has ID "${vitalparameter.observation-read-arterial-blood-pressure-id}"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '85354-9' and system = 'http://loinc.org' and version = '${vitalparameter.loinc-version}' and display.empty().not()).exists()" with error message 'The LOINC Code does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '75367002' and system = 'http://snomed.info/sct' and version = 'http://snomed.info/sct/11000274103/version/${vitalparameter.snomed-ct-version}' and display.empty().not()).exists()" with error message 'The SNOMED Code does not match the expected value'
    ...
    And FHIR current response body evaluates the FHIRPath "component.where(code.coding.where(system = 'http://loinc.org' and code = '8480-6' and version = '${vitalparameter.loinc-version}' and display.empty().not()).exists() and value.where(value ~ 107 and code = 'mm[Hg]' and system = 'http://unitsofmeasure.org').exists()).exists()" with error message 'The systolic blood pressure component does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "component.where(code.coding.where(system = 'http://loinc.org' and code = '8462-4' and version = '${vitalparameter.loinc-version}' and display.empty().not()).exists() and value.where(value ~ 60 and code = 'mm[Hg]' and system = 'http://unitsofmeasure.org').exists()).exists()" with error message 'The diastolic blood pressure component does not match the expected value'
  ...
  Scenario: Read optional Method from Observation
    Then Get FHIR resource at "http://fhirserver/Observation/${vitalparameter.observation-read-arterial-blood-pressure-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath "method.coding.where(code = '113011001' and system = 'http://snomed.info/sct' and version = 'http://snomed.info/sct/11000274103/version/${vitalparameter.snomed-ct-version}' and display.empty().not()).exists()" with error message 'The method does not match the expected value'
