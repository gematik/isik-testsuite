...
Feature: Testing search parameters against a resource of type Encounter, according to the definition of ISiKCapabilityStatementStammdatenRolle (@Encounter-Search @Terminplanung)
  ...
  Scenario: Search for the encounter by ID
    When Get FHIR resource at "http://fhirserver/Encounter/?_id=${data.terminplanung-encounter-id}" with content type "xml"
    And FHIR current response body is a valid CORE resource and conforms to profile "https://hl7.org/fhir/StructureDefinition/Bundle"
    And response bundle contains resource with ID "${data.terminplanung-encounter-id}" with error message "The requested Encounter ${data.terminplanung-encounter-id} is not contained in the response bundle."
  ...
  Scenario: Search for the Encounter by Status and ID
    When Get FHIR resource at "http://fhirserver/Encounter/?status=in-progress&_id=${data.terminplanung-encounter-id}" with content type "json"
  ...
  Scenario: Search for the Encounter by Patient and Encounter ID
    When Get FHIR resource at "http://fhirserver/Encounter/?patient=Patient/${data.patient-read-id}&_id=${data.terminplanung-encounter-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
  ...
  Scenario: Negative Search for the Encounter by Admission Date and Patient
    ...
    And bundle does not contain resource "Encounter" with ID "${data.terminplanung-encounter-id}" with error message "The requested encounter ${data.terminplanung-encounter-id} must not be returned here"
  ...
  Scenario: Search for the Encounter by Identifier
    When Get FHIR resource at "http://fhirserver/Encounter/?identifier=${data.terminplanung-encounter-identifier-system}%7C${data.terminplanung-encounter-identifier-value}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
    And FHIR current response body evaluates the FHIRPath "entry.resource.all(identifier.where(value = '${data.terminplanung-encounter-identifier-value}' and system = '${data.terminplanung-encounter-identifier-system}').exists())" with error message 'There are search results, but they do not fully match the search criteria'

  Scenario: Search for the Encounter by Identifier System
    When Get FHIR resource at "http://fhirserver/Encounter/?identifier=${data.terminplanung-encounter-identifier-system}%7C" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
    And FHIR current response body evaluates the FHIRPath "entry.resource.all(identifier.where(system = '${data.terminplanung-encounter-identifier-system}').exists())" with error message 'There are search results, but they do not fully match the search criteria'
