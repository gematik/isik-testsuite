...
Feature: Testing search parameters against a resource of type ValueSet (@ValueSet-Search)
  ...
  Scenario: Search for ValueSet by ID
    When Get FHIR resource at "http://fhirserver/ValueSet/?_id=${terminplanung.valueset-read-id}" with content type "xml"
    ...
    And response bundle contains resource with ID "${terminplanung.valueset-read-id}" with error message "The requested ValueSet ${terminplanung.valueset-read-id} is not contained in the response bundle"
  ...
  Scenario Outline: Search for ValueSet by additional search parameters and ID
    When Get FHIR resource at "http://fhirserver/ValueSet/?<searchParameter>=<searchValue>&_id=${terminplanung.valueset-read-id}" with content type "<contentType>"
  ...
  Scenario: Optional Search for ValueSet that have the given Tag, group by _count
    When Post FHIR search request to "http://fhirserver/ValueSet/_search" with content type "xml" and parameters:
      | _tag                                  | _count               |
      | ${terminplanung.tag-system}\|${terminplanung.tag-value} | ${terminplanung.search-count} |
  ...
  Scenario: Optional Search for ValueSet by context type, group by _count
    When Get FHIR resource at "http://fhirserver/ValueSet/?_count=${terminplanung.search-count}&context-type-value=focus%24http%3A%2F%2Fhl7.org%2Ffhir%2Fresource-types%7CEncounter" with content type "xml"
    Then TGR find the last request
