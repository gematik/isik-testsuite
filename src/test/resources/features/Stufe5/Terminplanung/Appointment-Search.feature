...
Feature: Testing search parameters against the Appointment resource (@Appointment-Search)
  ...
  @Optional
  Scenario: Search for the Appointment by Tag and ID
    When Get FHIR resource at "http://fhirserver/Appointment/?_tag=${data.tag-system}%7C${data.tag-value}&_id=${data.appointment-read-id}" with content type "xml"
  ...
  Scenario: Search for the Appointment that belong to a Patient, by Status
    Then Get FHIR resource at "http://fhirserver/Appointment/?status=cancelled&actor=Patient/${data.appointment-patient-id}" with content type "json"
  ...
  Scenario: Search for the Appointment that belong to a Patient, by Service Type
    Then Get FHIR resource at "http://fhirserver/Appointment/?service-type=${data.schedule-read-servicetype-code}&actor=Patient/${data.appointment-patient-id}" with content type "json"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.ofType(Appointment).count() > 0' with error message 'No search results were found'
  ...
  Scenario: Search for the Appointment that belong to a Patient, by Specialty
    Then Get FHIR resource at "http://fhirserver/Appointment/?actor=Patient/${data.appointment-patient-id}&specialty=http://ihe-d.de/CodeSystems/AerztlicheFachrichtungen%7CNEUR" with content type "json"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.ofType(Appointment).count() > 0' with error message 'No search results were found'
  ...
  Scenario: Search for the Appointment that belong to a Patient, by Date
    Then Get FHIR resource at "http://fhirserver/Appointment/?date=${data.slot-read-start}&actor=Patient/${data.appointment-patient-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.ofType(Appointment).count() > 0' with error message 'No search results were found'
  ...
  Scenario: Search for the Appointment that belong to a Patient, by Slot
    Then Get FHIR resource at "http://fhirserver/Appointment/?slot=${data.slot-read-id}&actor=Patient/${data.appointment-patient-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.ofType(Appointment).count() > 0' with error message 'No search results were found'
