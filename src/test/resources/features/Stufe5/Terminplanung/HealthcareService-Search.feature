...
Feature: Testing search parameters against the HealthcareService resource (@HealthcareService-Search)
  ...
  Scenario: Search for the HealthcareService by ID
    Then Get FHIR resource at "http://fhirserver/HealthcareService/?_id=${terminplanung.healthcareservice-read-id}" with content type "xml"
    And response bundle contains resource with ID "${terminplanung.healthcareservice-read-id}" with error message "The requested HealthcareService ${terminplanung.healthcareservice-read-id} is not contained in the response bundle"
  ...
  Scenario: Optional Search for the HealthcareService by Tag and ID
    When Get FHIR resource at "http://fhirserver/HealthcareService/?_tag=${terminplanung.tag-system}%7C${terminplanung.tag-value}&_id=${terminplanung.healthcareservice-read-id}" with content type "xml"
  ...
  Scenario: Search for the HealthcareService by Active State and ID
    Then Get FHIR resource at "http://fhirserver/HealthcareService/?active=true&_id=${terminplanung.healthcareservice-read-id}" with content type "json"
  ...
  Scenario: Search for the HealthcareService by Service Type
    Then Get FHIR resource at "http://fhirserver/HealthcareService/?service-type=${terminplanung.healthcareservice-read-servicetype-code}" with content type "json"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.ofType(HealthcareService).count() > 0' with error message 'No search results were found'
    And FHIR current response body evaluates the FHIRPath "entry.resource.ofType(HealthcareService).all(type.coding.where(code='${terminplanung.healthcareservice-read-servicetype-code}').exists())" with error message 'There are search results, but they do not fully match the search criteria.'
  ...
  Scenario: Search for the HealthcareService by Name
    Then Get FHIR resource at "http://fhirserver/HealthcareService/?name=${terminplanung.healthcareservice-search-name}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.ofType(HealthcareService).count() > 0' with error message 'No search results were found'
    And FHIR current response body evaluates the FHIRPath "entry.resource.ofType(HealthcareService).all(name.contains('${terminplanung.healthcareservice-read-name}'))" with error message 'There are search results, but they do not fully match the search criteria.'
