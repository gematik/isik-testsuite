...
Feature: Read a resource of type Encounter that has a valid Appointment reference (@Encounter-Appointment-Read)
  ...
  Scenario: Read and Validate planned Encounter by its ID
    When Get FHIR resource at "http://fhirserver/Encounter/${terminplanung.encounter-read-appointment-id}" with content type "xml"
    ...
    And resource has ID "${terminplanung.encounter-read-appointment-id}" with error message "The returned Encounter resource has not the expected ID"
    And TGR current response with attribute "$..status.value" matches "planned"
    And FHIR current response body evaluates the FHIRPath "identifier.where(system = '${terminplanung.encounter-read-appointment-identifier-system}' and value='${terminplanung.encounter-read-appointment-identifier-value}').exists()" with error message 'The Encounter does not contain the correct admission number'
    ...
    And element "subject" references resource with ID "${terminplanung.appointment-patient-id}" with error message "The referenced Patient does not match the expected value"
    And element "appointment" references resource with ID "${terminplanung.appointment-read-id}" with error message "The referenced Appointment does not match the expected value"
    ...
    And FHIR current response body evaluates the FHIRPath "serviceProvider.identifier.value = '${terminplanung.encounter-read-appointment-serviceprovider-identifier-value}'" with error message 'The service provider identifier value does not match the expected value'
