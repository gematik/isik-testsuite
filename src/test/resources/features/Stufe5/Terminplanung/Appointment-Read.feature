...
Feature: Read Information from a resource of type Appointment (@Appointment-Read)
  ...
  Scenario: Read an Appointment by ID
    Then Get FHIR resource at "http://fhirserver/Appointment/${terminplanung.appointment-read-id}" with content type "xml"
    And resource has ID "${terminplanung.appointment-read-id}"
    ...
    And FHIR current response body evaluates the FHIRPath "serviceType.coding.where(code='${terminplanung.schedule-read-servicetype-code}' and system = '${terminplanung.schedule-read-servicetype-system}').exists()" with error message 'The appointment service type does not match the expected value'
    ...
    And FHIR current response body evaluates the FHIRPath "start.toString().contains('${terminplanung.slot-read-start}') or start ~ @${terminplanung.slot-read-start}" with error message 'The start time does not match the expected value'
    ...
    And element "slot" references resource with ID "${terminplanung.slot-read-id}" with error message "The linked slot does not match the expected value"
    ...
    And FHIR current response body evaluates the FHIRPath "participant.actor.where(reference.replaceMatches('/_history/.+','').matches('\\b${terminplanung.appointment-patient-id}$') and display.exists()).exists()" with error message 'The participant does not match the expected value'
