...
Feature: Testing search parameters against a resource of type Practitioner, according to the definition of ISiKCapabilityStatementLeistungserbringerRolle (@Practitioner-Search)
  ...
  Scenario: Search for Practitioner resource by ID
    When Get FHIR resource at "http://fhirserver/Practitioner/?_id=${medikation.medication-practitioner-id}" with content type "xml"
    And response bundle contains resource with ID "${medikation.medication-practitioner-id}" with error message "The requested Practitioner resource ${medikation.medication-practitioner-id} is not contained in the response bundle"
  ...
  Scenario: Search for Practitioners that have the given Name "Walter", group by _count
    When Post FHIR search request to "http://fhirserver/Practitioner/_search" with content type "xml" and parameters:
      | given  | _count               |
      | Walter | ${medikation.search-count} |
  ...
  Scenario: Search for Practitioners that have the family Name "Musterarzt", group by _count
    When Post FHIR search request to "http://fhirserver/Practitioner/_search" with content type "xml" and parameters:
      | family     | _count               |
      | Musterarzt | ${medikation.search-count} |
    When Get FHIR resource at "http://fhirserver/Practitioner/?family=Musterarzt&_id=${medikation.medication-practitioner-id}" with content type "json"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0 and entry.resource.count() <= ${medikation.search-count}' with error message 'Invalid search results were found'
  ...
  Scenario: (Negative Test) Search for Practitioners that have the given Name "NotExistingName", group by _count
    When Post FHIR search request to "http://fhirserver/Practitioner/_search" with content type "xml" and parameters:
      | given           | _count               |
      | NotExistingName | ${medikation.search-count} |
    When Get FHIR resource at "http://fhirserver/Practitioner/?given=Max&_id=${medikation.medication-practitioner-id}" with content type "xml"
  ...
  Scenario: Optional Search for Practitioners that have the Address that contains "Musterweg", group by _count
    When Post FHIR search request to "http://fhirserver/Practitioner/_search" with content type "xml" and parameters:
      ...
      | Musterweg        | ${medikation.search-count} |
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0 and entry.resource.count() <= ${medikation.search-count}' with error message 'Invalid search results were found'
