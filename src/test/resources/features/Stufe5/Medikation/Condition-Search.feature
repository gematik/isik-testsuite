...
Feature: Testing search parameters against a resource of type Condition, according to the definition of ISiKCapabilityStatementKlinischeRolle (@Condition-Search)
  ...
  Scenario: Search for the Condition by ID
    When Get FHIR resource at "http://fhirserver/Condition/?_id=${data.medication-condition-active-id}" with content type "xml"
    And FHIR current response body is a valid CORE resource and conforms to profile "https://hl7.org/fhir/StructureDefinition/Bundle"
    And response bundle contains resource with ID "${data.medication-condition-active-id}" with error message "The requested Condition ${data.medication-condition-active-id} is not contained in the response bundle"
  ...
  Scenario: Search for Conditions by Patient (Search Parameter 'patient'), group by _count
    When Get FHIR resource at "http://fhirserver/Condition/?patient=Patient/${data.medication-patient-id}&_count=${data.search-count}" with content type "xml"
  ...
  Scenario: Search for Conditions by Encounter, group by _count
    When Get FHIR resource at "http://fhirserver/Condition/?encounter=Encounter/${data.medication-encounter-id}&_count=${data.search-count}" with content type "json"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0 and entry.resource.count() <= ${data.search-count}' with error message 'No valid search results were found'
    And element "encounter" in all bundle resources references resource with ID "${data.medication-encounter-id}"

  Scenario: Search for Conditions by Recorded Date with 'ge' Modifier and Patient ('patient'), group by _count
    When Get FHIR resource at "http://fhirserver/Condition/?recorded-date=ge2021-02-12&patient=Patient/${data.medication-patient-id}&_count=${data.search-count}" with content type "json"
  ...
  Scenario: Optional Search for Conditions by Patient (Search Parameter 'subject'), group by _count
    When Get FHIR resource at "http://fhirserver/Condition/?subject=Patient/${data.medication-patient-id}&_count=${data.search-count}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0 and entry.resource.count() <= ${data.search-count}' with error message 'No valid search results were found'
    And element "subject" in all bundle resources references resource with ID "${data.medication-patient-id}"

  @Optional
  Scenario: Optional Search for Conditions by Clinical Status 'active', group by _count
    When Get FHIR resource at "http://fhirserver/Condition/?clinical-status=active&_count=${data.search-count}" with content type "json"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0 and entry.resource.count() <= ${data.search-count}' with error message 'No valid search results were found'
  ...
  Scenario: Optional Search for Conditions by Category 'encounter-diagnosis', group by _count
    When Get FHIR resource at "http://fhirserver/Condition/?category=encounter-diagnosis&_count=${data.search-count}" with content type "json"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0 and entry.resource.count() <= ${data.search-count}' with error message 'No valid search results were found'
