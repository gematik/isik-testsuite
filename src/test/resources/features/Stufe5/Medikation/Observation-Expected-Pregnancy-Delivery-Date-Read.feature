...
Feature: Read information from a resource of type Observation for the expected pregnancy delivery date (@Observation-Expected-Pregnancy-Delivery-Date-Read)
  ...
  Scenario: Read and Validate expected pregnancy delivery date Observation by its ID
    When Get FHIR resource at "http://fhirserver/Observation/${medikation.medication-observation-expected-pregnancy-delivery-date-read-id}" with content type "xml"
    ...
    And resource has ID "${medikation.medication-observation-expected-pregnancy-delivery-date-read-id}" with error message "The ID does not match the expected value"
    And TGR current response with attribute "$..status.value" matches "final"
    And FHIR current response body evaluates the FHIRPath "code.coding.where(system = 'http://loinc.org' and code = '11779-6' and version = '${medikation.loinc-version}' and display.empty().not()).exists()" with error message 'The LOINC code for the expected pregnancy delivery date does not match the expected value'
