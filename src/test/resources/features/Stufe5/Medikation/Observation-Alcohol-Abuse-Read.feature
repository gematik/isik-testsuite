...
Feature: Read information from a resource of type Observation for alcohol abuse (@Observation-Alcohol-Abuse-Read)
  ...
  Scenario: Read and Validate alcohol abuse Observation by its ID
    When Get FHIR resource at "http://fhirserver/Observation/${data.medication-observation-alcohol-abuse-read-id}" with content type "xml"
    ...
    And resource has ID "${data.medication-observation-alcohol-abuse-read-id}" with error message "The ID does not match the expected value"
