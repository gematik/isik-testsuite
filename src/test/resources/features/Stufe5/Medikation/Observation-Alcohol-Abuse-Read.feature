...
Feature: Read information from a resource of type Observation for alcohol abuse (@Observation-Alcohol-Abuse-Read)
  ...
  Scenario: Precondition
    ...
    """
      ...
      - The ID of the corresponding FHIR resource for this test dataset must be stored in the configuration variable 'medication-observation-alcohol-abuse-read-id'.

      Create an Observation resource with these fields:
      * Id: the value of the configuration variable 'medication-observation-alcohol-abuse-read-id'
  ...
  Scenario: Read and Validate alcohol abuse Observation by its ID
    When Get FHIR resource at "http://fhirserver/Observation/${medikation.medication-observation-alcohol-abuse-read-id}" with content type "xml"
    ...
    And resource has ID "${medikation.medication-observation-alcohol-abuse-read-id}" with error message "The ID does not match the expected value"
    And TGR current response with attribute "$..status.value" matches "final"
    And FHIR current response body evaluates the FHIRPath "code.coding.where(system = 'http://snomed.info/sct' and code = '15167005' and version = 'http://snomed.info/sct/11000274103/version/${medikation.snomed-ct-version}' and display.empty().not()).exists()" with error message 'The SNOMED CT code for alcohol abuse does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "code.coding.where(system = 'http://loinc.org' and code = '74043-1' and version = '${medikation.loinc-version}' and display.empty().not()).exists()" with error message 'The LOINC code for alcohol abuse does not match the expected value'
