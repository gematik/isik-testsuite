...
Feature: Update a resource of type Medication List (@List-Update)
  ...
  Scenario: Update a Medication List resource
    ...
    And TGR send PUT request to "http://fhirserver/List/${data.list-update-id}" with body "!{file('src/test/resources/features/Stufe5/Medikation/fixtures/List-Update-Fixture.json')}"
    And TGR find the last request
    Then TGR current response with attribute "$.responseCode" matches "200"
    And TGR send empty GET request to "http://fhirserver/List/${data.list-update-id}"
    And TGR find the last request
    Then FHIR current response body evaluates the FHIRPath "entry.where(item.reference.replaceMatches('/_history/.+','').matches('MedicationStatement/${data.medicationstatement-read-id}$')).exists()" with error message 'List entry was not found'
    Then FHIR current response body evaluates the FHIRPath "entry.where(item.reference.replaceMatches('/_history/.+','').matches('MedicationStatement/${data.medicationstatement-read-extended-id}$')).exists()" with error message 'List entry was not found'