...
Feature: Read information from a resource of type Observation for breastfeeding status (@Observation-Breastfeeding-Read)
  ...
  Scenario: Read and Validate breastfeeding Observation by its ID
    When Get FHIR resource at "http://fhirserver/Observation/${medikation.medication-observation-breastfeeding-read-id}" with content type "xml"
    ...
    And resource has ID "${medikation.medication-observation-breastfeeding-read-id}" with error message "The ID does not match the expected value"
    And TGR current response with attribute "$..status.value" matches "final"
    And FHIR current response body evaluates the FHIRPath "code.coding.where(system = 'http://snomed.info/sct' and code = '413712001' and version = 'http://snomed.info/sct/11000274103/version/${medikation.snomed-ct-version}' and display.empty().not()).exists()" with error message 'The SNOMED CT code for breastfeeding status does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "code.coding.where(system = 'http://loinc.org' and code = '63895-7' and version = '${medikation.loinc-version}' and display.empty().not()).exists()" with error message 'The LOINC code for breastfeeding status does not match the expected value'
    ...
    And FHIR current response body evaluates the FHIRPath "value.coding.where(system = 'http://loinc.org' and code = 'LA29252-6' and version = '${medikation.loinc-version}' and display.empty().not()).exists()" with error message 'The breastfeeding status value does not match the expected value'
