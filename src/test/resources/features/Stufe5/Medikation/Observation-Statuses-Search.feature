...
Feature: Testing search parameters against Observation resources for status-related observations (@Observation-Statuses-Search)
  ...
  Scenario Outline: Search for Observation resources by ID
    ...
    Examples:
      | observationId                                                           |
      | ${medikation.medication-observation-alcohol-abuse-read-id}                    |
      | ${medikation.medication-observation-breastfeeding-read-id}                    |
      | ${medikation.medication-observation-pregnancy-status-read-id}                 |
      | ${medikation.medication-observation-expected-pregnancy-delivery-date-read-id} |
      | ${medikation.medication-observation-smoking-status-read-id}                   |

  Scenario: Search for Observation resources by Status "final" and Encounter
    When Get FHIR resource at "http://fhirserver/Observation/?status=http://hl7.org/fhir/observation-status%7Cfinal&encounter=Encounter/${medikation.medication-encounter-id}" with content type "xml"
    ...
    And FHIR current response body evaluates the FHIRPath 'entry.resource.all(encounter.reference.replaceMatches("/_history/.+","").matches("\\b${medikation.medication-encounter-id}$"))' with error message 'There are search results, but they do not fully match the encounter search criteria'

  Scenario Outline: Search for Observation resources by Code and Encounter
    When Get FHIR resource at "http://fhirserver/Observation/?code=http://loinc.org%7C<loincCode>&encounter=Encounter/${medikation.medication-encounter-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
    ...
    Examples:
      ...
      | 74043-1   | ${medikation.medication-observation-alcohol-abuse-read-id}                    |
      | 63895-7   | ${medikation.medication-observation-breastfeeding-read-id}                    |
      | 82810-3   | ${medikation.medication-observation-pregnancy-status-read-id}                 |
      | 11779-6   | ${medikation.medication-observation-expected-pregnancy-delivery-date-read-id} |
      | 72166-2   | ${medikation.medication-observation-smoking-status-read-id}                   |

  Scenario: Search for Observation resources by Patient (search parameter 'patient')
    When Get FHIR resource at "http://fhirserver/Observation/?patient=Patient/${medikation.medication-observation-patient-read-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
    And element "subject" in all bundle resources references resource with ID "Patient/${medikation.medication-observation-patient-read-id}"

  Scenario: Search for Observation resources by Date and Encounter
    When Get FHIR resource at "http://fhirserver/Observation/?date=2026-01-06&encounter=Encounter/${medikation.medication-encounter-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
  ...
  Scenario: Search for Observation resources by Encounter and Patient
    When Get FHIR resource at "http://fhirserver/Observation/?encounter=Encounter/${medikation.medication-encounter-id}&patient=Patient/${medikation.medication-observation-patient-read-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
    And element "subject" in all bundle resources references resource with ID "Patient/${medikation.medication-observation-patient-read-id}"
  ...
  Scenario: Optional Search for Observation resources that have the given Tag, group by _count
    ...
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0 and entry.resource.count() <= ${medikation.search-count}' with error message 'Invalid search results were found'
  ...
  Scenario: Optional Search for Observation resources by Subject (search parameter 'subject')
    When Get FHIR resource at "http://fhirserver/Observation/?subject=Patient/${medikation.medication-observation-patient-read-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
    And element "subject" in all bundle resources references resource with ID "Patient/${medikation.medication-observation-patient-read-id}"
