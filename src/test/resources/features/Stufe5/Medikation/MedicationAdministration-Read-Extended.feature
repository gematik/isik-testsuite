...
Feature: Read Information from a resource of type MedicationAdministration with extended data (@MedicationAdministration-Read-Extended)
  ...
  Scenario: Read and Validate the MedicationAdministration with extended data by its ID
    ...
    And FHIR current response body evaluates the FHIRPath "subject.identifier.value = '${data.medication-patient-identifier}'" with error message "The subject identifier does not match the expected value"
    ...
    And FHIR current response body evaluates the FHIRPath "performer.actor.reference.replaceMatches('/_history/.+','').matches('Practitioner/${data.medication-practitioner-id}$')" with error message 'The performer does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "performer.actor.identifier.value = '${data.medication-practitioner-identifier}'" with error message 'The performer identifier does not match the expected value'
