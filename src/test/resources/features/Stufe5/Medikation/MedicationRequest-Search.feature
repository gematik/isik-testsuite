...
Feature: Testing search parameters against a resource of type MedicationRequest (@MedicationRequest-Search)
  ...
  Scenario: Search for the MedicationRequest by ID
    ...
    And response bundle contains resource with ID "${data.medicationrequest-read-id}" with error message "The requested resource with ID ${data.medicationrequest-read-id} is not contained in the response bundle"
  ...
  Scenario: Search for the MedicationRequest that belong to a Patient, by Authored Date
    Then Get FHIR resource at "http://fhirserver/MedicationRequest/?authoredon=gt2026-01-01&patient=Patient/${data.medication-patient-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
  ...
  Scenario: Search for the MedicationRequest that belong to a Patient, by Code
    Then Get FHIR resource at "http://fhirserver/MedicationRequest/?code=R01AA04&patient=Patient/${data.medication-patient-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
  ...
  Scenario: Search for the MedicationRequest that belong to a Patient, by Date
    Then Get FHIR resource at "http://fhirserver/MedicationRequest/?date=gt2026-01-01&patient=Patient/${data.medication-patient-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
  ...
  Scenario: Search for the MedicationRequest that belong to a Patient, by Encounter
    Then Get FHIR resource at "http://fhirserver/MedicationRequest/?encounter=Encounter/${data.medication-encounter-id}&patient=Patient/${data.medication-patient-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
  ...
  Scenario: Search for the MedicationRequest that belong to a Patient, by Referenced Medication
    Then Get FHIR resource at "http://fhirserver/MedicationRequest/?medication=Medication/${data.medicationrequest-medication-id}&patient=Patient/${data.medication-patient-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
  ...
  Scenario: Search for the MedicationRequest that belong to a Patient, by Requester Identifier
    Then Get FHIR resource at "http://fhirserver/MedicationRequest/?requester.identifier=${data.medication-practitioner-identifier}&patient=Patient/${data.medication-patient-id}" with content type "json"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
    And FHIR current response body evaluates the FHIRPath "entry.resource.all(requester.identifier.value='${data.medication-practitioner-identifier}' or requester.reference.replaceMatches('/_history/.+','').matches('Practitioner/${data.medication-practitioner-id}$'))" with error message 'There are search results, but they do not fully match the search criteria'

  Scenario: Search for the MedicationRequest that belong to a Patient, by Status
    Then Get FHIR resource at "http://fhirserver/MedicationRequest/?status=completed&patient=Patient/${data.medication-patient-id}" with content type "json"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
