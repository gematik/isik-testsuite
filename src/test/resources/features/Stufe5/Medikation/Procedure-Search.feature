...
Feature: Testing search parameters against a resource of type Procedure, according to the definition of ISiKCapabilityStatementKlinischeRolle (@Procedure-Search)
  ...
  Scenario: Search for Procedure resource by ID
    When Get FHIR resource at "http://fhirserver/Procedure/?_id=${data.medication-procedure-id}" with content type "xml"
    And FHIR current response body is a valid CORE resource and conforms to profile "https://hl7.org/fhir/StructureDefinition/Bundle"
    And response bundle contains resource with ID "${data.medication-procedure-id}" with error message "The requested Procedure ${data.medication-procedure-id} is not contained in the response bundle"
  ...
  Scenario: Search for Procedures by Status 'completed' and Patient
    When Get FHIR resource at "http://fhirserver/Procedure/?status=completed&patient=Patient/${data.medication-patient-id}" with content type "xml"
    ...
    And element "subject" in all bundle resources references resource with ID "${data.medication-patient-id}"
    And response bundle contains resource with ID "${data.medication-procedure-id}" with error message "The requested Procedure ${data.medication-procedure-id} is not contained in the response bundle"

  Scenario: Search for Procedures by Category Code 387713003 and Patient, group by _count
    When Get FHIR resource at "http://fhirserver/Procedure/?patient=Patient/${data.medication-patient-id}&category=http://snomed.info/sct%7C387713003&_count=${data.search-count}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0 and entry.resource.count() <= ${data.search-count}' with error message 'No valid search results were found'
  ...
  Scenario: Search for Procedures by Code 5-470.11 (OPS) and Patient, group by _count
    When Get FHIR resource at "http://fhirserver/Procedure/?patient=Patient/${data.medication-patient-id}&code=http://fhir.de/CodeSystem/bfarm/ops%7C5-470.11&_count=${data.search-count}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0 and entry.resource.count() <= ${data.search-count}' with error message 'No valid search results were found'
  ...
  Scenario: Search for Procedures by Performed Date and Patient, group by _count
    When Get FHIR resource at "http://fhirserver/Procedure/?date=le2050-01-01&patient=Patient/${data.medication-patient-id}&_count=${data.search-count}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0 and entry.resource.count() <= ${data.search-count}' with error message 'No valid search results were found'
  ...
  Scenario: Search for Procedures by Encounter and Patient, group by _count
    When Get FHIR resource at "http://fhirserver/Procedure/?encounter=Encounter/${data.encounter-read-in-progress-id}&patient=Patient/${data.medication-patient-id}&_count=${data.search-count}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0 and entry.resource.count() <= ${data.search-count}' with error message 'No valid search results were found'
  ...
  Scenario: Negative search for Procedures by Status and Patient, group by _count
    When Get FHIR resource at "http://fhirserver/Procedure/?_id:not=${data.medication-procedure-id}&status:not=completed&patient=Patient/${data.medication-patient-id}&_count=${data.search-count}" with content type "xml"
    And bundle does not contain resource "Procedure" with ID "${data.medication-procedure-id}" with error message "There are search results, but they do not fully match the search criteria"

  @Optional
  Scenario: Optional Search for Procedures resource by Tag, group by count
    When Get FHIR resource at "http://fhirserver/Procedure/?_tag=${data.tag-system}%7C${data.tag-value}&_id=${data.medication-procedure-id}&_count=${data.search-count}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0 and entry.resource.count() <= ${data.search-count}' with error message 'No valid search results were found'
  ...
  Scenario: Optional Search for Procedures by Subject reference
    When Get FHIR resource at "http://fhirserver/Procedure/?subject=Patient/${data.medication-patient-id}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0 and entry.resource.count() <= ${data.search-count}' with error message 'No valid search results were found'
    And element "subject" in all bundle resources references resource with ID "${data.medication-patient-id}"