...
Feature: Testing search parameters against a resource of type Encounter, according to the definition of ISiKCapabilityStatementLeistungserbringerRolle (@Encounter-Search @Medikation)
  ...
  Scenario: Search for the encounter by Location
    When Get FHIR resource at "http://fhirserver/Encounter/?location=Location/${medikation.medication-encounter-location}" with content type "xml"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
    And FHIR current response body evaluates the FHIRPath 'entry.resource.all(location.location.reference.replaceMatches("/_history/.+","").matches("\\b${medikation.medication-encounter-location}$"))' with error message 'There are search results, but they do not fully match the search criteria'
  ...
  Scenario: Search for the encounter by Service Provider
    When Get FHIR resource at "http://fhirserver/Encounter/?service-provider=${medikation.medication-encounter-service-provider}" with content type "json"
    And FHIR current response body evaluates the FHIRPath 'entry.resource.count() > 0' with error message 'No search results were found'
    And FHIR current response body evaluates the FHIRPath 'entry.resource.all(serviceProvider.reference.replaceMatches("/_history/.+","").matches("\\b${medikation.medication-encounter-service-provider}$"))' with error message 'There are search results, but they do not fully match the search criteria'
