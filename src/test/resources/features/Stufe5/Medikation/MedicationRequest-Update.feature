...
Feature: Update a resource of type MedicationRequest (@MedicationRequest-Update)
  ...
  Scenario: Update a MedicationRequest resource
    ...
    When TGR send PUT request to "http://fhirserver/MedicationRequest/${data.medicationrequest-update-id}" with body "!{file('src/test/resources/features/Stufe5/Medikation/fixtures/MedicationRequest-Update-Fixture.json')}"
