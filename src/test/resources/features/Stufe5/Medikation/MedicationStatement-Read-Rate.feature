...
Feature: Read Information from a resource of type MedicationStatement with Rate data (@MedicationStatement-Read-Rate)
  ...
  Scenario: Read and Validate the MedicationStatement with Rate data by its ID
    ...
    And FHIR current response body evaluates the FHIRPath "dosage.route.coding.where(system='http://snomed.info/sct' and code='255560000' and version = 'http://snomed.info/sct/11000274103/version/${data.snomed-ct-version}' and display.empty().not()).exists()" with error message 'The route does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "dosage.site.coding.where(code = '6073002' and system = 'http://snomed.info/sct' and version = 'http://snomed.info/sct/11000274103/version/${data.snomed-ct-version}' and display.empty().not()).exists()" with error message 'The administration site does not match the expected value'
