...
Feature: Read Information from a resource of type AllergyIntolerance (@AllergyIntolerance-Read)
  ...
  Scenario: Read and Validate the AllergyIntolerance by its ID
    When Get FHIR resource at "http://fhirserver/AllergyIntolerance/${medikation.medication-allergyintolerance-read-id}" with content type "xml"
    ...
    And resource has ID "${medikation.medication-allergyintolerance-read-id}" with error message "The ID does not match the expected value"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '256262001' and display = 'Betula pendula pollen' and system = 'http://snomed.info/sct' and version = 'http://snomed.info/sct/11000274103/version/${medikation.snomed-ct-version}').exists()" with error message "The Code does not meet the preconditions."
    And FHIR current response body evaluates the FHIRPath "reaction.manifestation.coding.where(code = '76067001' and display = 'Sneezing (finding)' and system = 'http://snomed.info/sct' and version = 'http://snomed.info/sct/11000274103/version/${medikation.snomed-ct-version}').exists()" with error message "The Manifestation Coding does not meet the preconditions."
    And FHIR current response body evaluates the FHIRPath "reaction.exposureRoute.coding.where(code = '14910006' and display = 'Inspiration' and system = 'http://snomed.info/sct' and version = 'http://snomed.info/sct/11000274103/version/${medikation.snomed-ct-version}').exists()" with error message "The Exposure Route Coding does not meet the preconditions."
    ...
    And element "patient" references resource with ID "Patient/${medikation.medication-allergyintolerance-read-patient-id}" with error message "The referenced patient does not match the expected value"
    # Validate the referenced resources at the end -> Tiger performs new Requests
    And referenced "Patient" resource with id "${medikation.medication-allergyintolerance-read-patient-id}" conforms to a valid v5 "ISiKPatient" profile
