...
Feature: Read information from a resource of type Observation for pregnancy status (@Observation-Pregnancy-Status-Read)
  ...
  Scenario: Read and Validate pregnancy status Observation by its ID
    When Get FHIR resource at "http://fhirserver/Observation/${data.medication-observation-pregnancy-status-read-id}" with content type "xml"
    ...
    And resource has ID "${data.medication-observation-pregnancy-status-read-id}" with error message "The ID does not match the expected value"
    ...
    And element "hasMember" references resource with ID "Observation/${data.medication-observation-expected-pregnancy-delivery-date-read-id}" with error message "The referenced expected delivery date observation does not match the expected value"
