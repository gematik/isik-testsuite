...
Feature: Read information from a resource of type DocumentReference (@DocumentReference-Read)
  ...
  Scenario: Read document metadata by ID
    When Get FHIR resource at "http://fhirserver/DocumentReference/${dokument.documentreference-read-id}" with content type "json"
    And resource has ID "${dokument.documentreference-read-id}"
    ...
    And FHIR current response body evaluates the FHIRPath "masterIdentifier.where(system = '${dokument.documentreference-read-masteridentifier-system}' and value = '${dokument.documentreference-read-masteridentifier-value}').exists()" with error message 'The version-specific OID of the document does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "identifier.where(system = '${dokument.documentreference-read-externalidentifier-system}' and value = '${dokument.documentreference-read-externalidentifier-value}').exists()" with error message 'The identifier does not match the expected value'
    ...
    And FHIR current response body evaluates the FHIRPath "content.where(attachment.where(title.exists() and contentType = 'application/pdf' and (language = 'de' or language.startsWith('de-')) and url.exists() and creation = @2026-01-31T14:50:50+01:00).exists() and format.where(code = 'urn:ihe:iti:xds:2017:mimeTypeSufficient' and system = 'http://ihe.net/fhir/ihe.formatcode.fhir/CodeSystem/formatcode').exists()).exists()" with error message 'The attachment does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "content.attachment.url.contains('Binary/${dokument.documentreference-read-binary-id}')" with error message 'The Binary reference does not match the expected value'
    ...
    And element "subject" references resource with ID "${dokument.documentreference-read-patient-id}" with error message "The reference to Patient does not match the expected value"
    And element "context.encounter" references resource with ID "${dokument.documentreference-read-encounter-id}" with error message "The reference to Encounter does not match the expected value"
    And referenced "Encounter" resource with id "${dokument.documentreference-read-encounter-id}" conforms to a valid v5 "ISiKKontaktGesundheitseinrichtung" profile
    And referenced "Patient" resource with id "${dokument.documentreference-read-patient-id}" conforms to a valid v5 "ISiKPatient" profile

  Scenario: Read the Binary resource referenced in the DocumentReference resource
    When Get FHIR resource at "http://fhirserver/Binary/${dokument.documentreference-read-binary-id}" with content type "json"
    ...
    And resource has ID "${dokument.documentreference-read-binary-id}" with error message "The Binary ID does not match the expected value"
