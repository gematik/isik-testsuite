...
@ISiKCapabilityStatementMetadatenErzeugenRolle
...
Feature: Upload of a DocumentReference with POST Operation (Dokumentenbereitstellung) (@DocumentReference-Post)
  ...
  Scenario: POST a DocumentBundle with known patient and encounter
    ...
    And FHIR current response body evaluates the FHIRPath "masterIdentifier.where(system = 'urn:ietf:rfc:3986' and value = '${dokument.documentreference-post-master-identifier-value}').exists()" with error message 'The version-specific OID of the document does not match the expected value'
