...
@Labor
@Mandatory
@Observation-Read-Thrombozyten
Feature: Read Information from a resource of type Observation for platelet count (Thrombozyten) (@Observation-Read-Thrombozyten)
  ...
  Scenario: Precondition
    ...
    """
      ...
      - The ID of the corresponding FHIR resource for this test data set must be stored in the configuration variable 'observation-read-thrombozyten-id'.

      Create an Observation resource in your system with the following values:
      * Id: the value of the configuration variable 'observation-read-thrombozyten-id'
      ...
      * Note: "Unauffälliger Thrombozytenwert."
      * Specimen:
  ...
  Scenario: Read and Validate the Observation by its ID
    Then Get FHIR resource at "http://fhirserver/Observation/${labor.observation-read-thrombozyten-id}" with content type "xml"
    And resource has ID "${labor.observation-read-thrombozyten-id}"
    And FHIR current response body is a valid isik5 resource and conforms to profile "https://gematik.de/fhir/isik/StructureDefinition/ISiKLaboruntersuchungThrombozyten"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '26515-7' and system = 'http://loinc.org' and version = '${labor.loinc-version}' and display.empty().not()).exists()" with error message 'The LOINC code does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '365632008' and system = 'http://snomed.info/sct' and version = 'http://snomed.info/sct/11000274103/version/${labor.snomed-ct-version}' and display.empty().not()).exists()" with error message 'The SNOMED code does not match the expected value'
    ...
    And FHIR current response body evaluates the FHIRPath "value.where(value ~ 178 and code = '10*3/uL' and unit.exists() and system = 'http://unitsofmeasure.org').exists()" with error message 'The value does not match the expected value'
    And FHIR current response body evaluates the FHIRPath "referenceRange.where(low.value ~ 150 and low.code = '10*3/uL' and low.system = 'http://unitsofmeasure.org' and high.value ~ 400 and high.code = '10*3/uL' and high.system = 'http://unitsofmeasure.org' and type.coding.where(code = 'normal' and system = 'http://terminology.hl7.org/CodeSystem/referencerange-meaning').exists()).exists()" with error message 'The reference range does not match the expected value'
    ...
    And FHIR current response body evaluates the FHIRPath 'note.text = "Unauffälliger Thrombozytenwert."' with error message 'The note does not match the expected value'
