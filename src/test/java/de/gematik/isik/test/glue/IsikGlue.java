...
public class IsikGlue {
  ...
  @Steps private final RBelValidatorGlue rBelValidatorGlue = new RBelValidatorGlue();
  @Steps private final FhirPathValidationGlue fhirPathValidationGlue = new FhirPathValidationGlue();
  @Steps private final HttpGlueCode httpGlueCode = new HttpGlueCode();
  @Steps private final TigerGlue tigerGlue = new TigerGlue();
  ...
  public void getAndValidateResource(String address, String contentType) {
    ...
    httpGlueCode.setDefaultHeader("Accept", "application/fhir+" + contentType);
    httpGlueCode.sendEmptyRequest(Method.GET, URI.create(resolvedAddress));
  ...
  @When(
      "Get FHIR resource at {string} with content type {string} and with a valid patient bearer"
          + " token")
  public void getWithTokenAndValidateResource(String address, String contentType) {
    rBelValidatorGlue.tgrClearRecordedMessages();
    String resolvedAddress = TigerGlobalConfiguration.resolvePlaceholders(address);
    var accessToken =
        getAccessToken("src/test/resources/features/Stufe5/Connect/fixtures/bearer-patient.json");
    httpGlueCode.setDefaultHeader("Accept", "application/fhir+" + contentType);
    httpGlueCode.setDefaultHeader("Authorization", "Bearer " + accessToken);
    httpGlueCode.sendEmptyRequest(Method.GET, URI.create(resolvedAddress));
    rBelValidatorGlue.findLastRequest();
    rBelValidatorGlue.currentResponseMessageAttributeMatches("$.responseCode", "200");
    rBelValidatorGlue.currentResponseMessageAttributeMatches(
  ...
  private static @NonNull String getAccessToken(String tokenFile) throws RuntimeException {
    final var jwtToken =
        TigerGlobalConfiguration.resolvePlaceholders("!{file('" + tokenFile + "')}");
    try {
      final var jsonContent = new ObjectMapper().readTree(jwtToken);
      if (!jsonContent.has("access_token")) {
        throw new RuntimeException("Access Token is missing");
      }

      return jsonContent.asText("access_token");
